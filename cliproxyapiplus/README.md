...
- **CPA Usage Keeper**: https://github.com/Willxup/cpa-usage-keeper (固定构建 tag: `v1.11.3`)
...
`ghcr.io/naiba/cliproxyapiplus:keeper` 从 `Willxup/cpa-usage-keeper` 的固定 tag `v1.11.3` 构建，用于独立保存和展示 CLIProxyAPI 用量统计。运行前需要在 CLIProxyAPIPlus 的 `config.yaml` 中启用用量队列。`CPA_BASE_URL` 是 Keeper 访问 CPA 的服务端地址，`CPA_PUBLIC_URL` 是浏览器端“返回 CPA”的回跳地址；当浏览器实际使用的是其他域名、端口或路径时，请把它设为对应的公网可访问 URL。
...
2. Clone CPA Usage Keeper 固定 tag `v1.11.3` 并校验 commit 后构建 `keeper` 标签
3. 分别编译并注入版本信息
...
- `ghcr.io/naiba/cliproxyapiplus:keeper` - CPA Usage Keeper，固定从 `Willxup/cpa-usage-keeper` 的 `v1.11.3` tag 构建
